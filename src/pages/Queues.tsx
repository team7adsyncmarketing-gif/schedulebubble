  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(selectedJobs.map(id => 
        fetch(`https://schedulebubble.onrender.com/api/posts/queue/${id}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` },
          credentials: 'include'
        })
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-8">
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex bg-slate-200/60 dark:bg-slate-900/60 p-1 rounded-xl border border-slate-300 dark:border-slate-800">
          <button 
      ...
      {!loading && jobs.length > 0 && (
        <div className="flex items-center justify-between mb-6 bg-slate-100/80 dark:bg-slate-900/40 p-3 rounded-xl border border-slate-200 dark:border-slate-800/60">
              ...
              <div className={`flex flex-col h-full bg-white/70 dark:bg-slate-900/50 backdrop-blur-xl border rounded-2xl p-6 shadow-xl transition-all duration-300 ${isFailed ? 'border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.15)] hover:border-red-500/70' : 'border-slate-200 dark:border-slate-800/80 hover:border-indigo-500/30'} ${selectedJobs.includes(job._id) ? 'ring-2 ring-indigo-500/50 border-indigo-500/50 bg-indigo-50/50 dark:bg-slate-800/80' : ''}`}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300 capitalize">{platform?.name || job.platform}</span>
                  </div>
                ...
                >
                  <p className="text-slate-700 dark:text-slate-300 text-sm line-clamp-4 group-hover:text-slate-900 dark:group-hover:text-slate-200 transition-colors">{job.post?.content}</p>
                  {job.post?.mediaUrls && job.post.mediaUrls.length > 0 && (
                    <div className="mt-3 flex items-center gap-2 text-xs text-slate-500 bg-slate-100 dark:bg-slate-800/50 px-2 py-1 rounded w-fit border border-slate-200 dark:border-slate-700/50 group-hover:bg-slate-200 dark:group-hover:bg-slate-700/50 transition-colors">
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-800/80 mb-4">
                ...
                <div className="flex items-center gap-2 mt-auto">
                  {job.status !== 'published' && job.status !== 'failed' && (
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 px-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-xl transition border border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600"
                    >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 w-full max-w-lg shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl p-4 min-h-[120px] focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-xl p-3 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-8">
              <button
                ...
                className="flex-1 py-3 px-2 whitespace-nowrap bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-white font-semibold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition"
              >
                Discard
              </button>
                  ...
                  <div className="flex-1">
                    <Magnetic>
                      <button
                        ...
                        disabled={isSaving}
                        className="w-full py-3 px-2 whitespace-nowrap bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-white font-semibold rounded-xl hover:bg-slate-300 dark:hover:bg-slate-600 transition disabled:opacity-50"
                      >
                        Save Draft
                      </button>
                    </Magnetic>
      ...
      {viewingJob && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <button 
              onClick={() => setViewingJob(null)}
              className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-full transition shadow-md"
            >
              <X className="w-5 h-5" />
            </button>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white capitalize">{platformIcons[viewingJob.platform]?.name || viewingJob.platform}</h3>
            ...
            <div className="mb-6">
              <p className="text-slate-800 dark:text-slate-200 whitespace-pre-wrap leading-relaxed text-[15px]">{viewingJob.post?.content}</p>
            </div>
