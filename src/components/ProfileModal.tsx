...
export const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose, user, onUserUpdate, onLogout }) => {
  ...
  const handleProfilePictureUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('https://schedulebubble-zjof.onrender.com/api/users/profile-picture', {
        method: 'PUT',
  ...
  const handleUpdateName = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://schedulebubble-zjof.onrender.com/api/auth/update-profile', {
        method: 'PUT',
        headers: {
